...
#section-link("03 Type Unions", "", "Type unions") are the most common and allow the programmer to create a flexible variable that hold multiple different concrete types, while still being fully type checked. 

#section-link("06 Type Intersections", "", "Type intersections") allow the programmer to create a variable that can only contain values that satisfy multiple interfaces. This use case is less common, but allows variables to be entirely 
